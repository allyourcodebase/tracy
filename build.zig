...
pub fn build(b: *std.Build) !void {
    ...
    const ignore_memory_faults = b.option(bool, "ignore-memory-faults", "Ignore instrumentation errors from memory free events that do not have a matching allocation") orelse false;
    ...
    const link_system_debuginfod = if (debuginfod) b.systemIntegrationOption("debuginfod", .{ .default = true }) else false;
    ...
    if (link_system_debuginfod) tracy_client.root_module.linkSystemLibrary("debuginfod", .{}) else if (debuginfod) @panic("Unsupported");
    ...
    if (debuginfod) tracy_client.root_module.addCMacro("TRACY_DEBUGINFOD", "1");
    if (ignore_memory_faults) tracy_client.root_module.addCMacro("TRACY_IGNORE_MEMORY_FAULTS", "1");
    ...
    if (debuginfod) tracy_profiler.root_module.addCMacro("TRACY_DEBUGINFOD", "1");
    if (ignore_memory_faults) tracy_profiler.root_module.addCMacro("TRACY_IGNORE_MEMORY_FAULTS", "1");
